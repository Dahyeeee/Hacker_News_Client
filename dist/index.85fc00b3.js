...
})({"lrGur":[function(require,module,exports) {
"use strict";
...
module.bundle.HMR_BUNDLE_ID = "7950973e85fc00b3";
...
},{}],"99uh0":[function(require,module,exports) {
...
const newsFeedPage = ()=>{
    ...
    <li class='mb-4 '>
      ...
      ${newsFeed[i].read ? "[READ]" : ""} ${newsFeed[i].title} (${newsFeed[i].comments_count})
      </a>
    </li>
   `);
...
const articlePage = ()=>{
    ...
    for(let i1 = 0; i1 < store.feeds.length; i1++)if (store.feeds[i1].id == +id) {
    ...
    function makeComments(comments, call) {
      ...
      `);
            if (comments[i].comments.length > 0) commentSting.push(makeComments(comments[i].comments, call + 1));
        }
    ...
    template = template.replace("{{__comments__}}", makeComments(newsContent.comments, 0));
    container.innerHTML = template;
};
function router() {
...
//# sourceMappingURL=index.85fc00b3.js.map
